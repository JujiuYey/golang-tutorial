...
`timer.C` 是一个 channel(07 章的老朋友):时间到了,里面会来一个值,`<-timer.C` 就是在等这声铃响。它天然能进 `select`,07-06 的超时模式用的 `time.After(d)` 就是它的便捷包装。高频循环里别反复 `time.After`(每次都新建 Timer),复用 `NewTimer` + `Reset`。
...
适合简单等待和测试模拟。🕳️ 坑:别用 Sleep 等 goroutine 干完活("睡 1 秒应该够它跑完了")——这是 07 章反复强调的反模式,时间估短了出 bug,估长了白等。等任务完成用 `WaitGroup`、channel 或 context。
