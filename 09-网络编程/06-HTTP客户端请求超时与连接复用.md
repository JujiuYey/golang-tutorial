...
func fetch(ctx context.Context, client *http.Client, rawURL string) ([]byte, error) {
	...
	defer resp.Body.Close() // ③预约关 body(06-03 的套路)
...
🕳️ 坑：`defer resp.Body.Close()` 不能写在 `if err != nil` 之前——`Do` 失败时 `resp` 是 nil，Close 一个 nil 直接 panic。先确认资源到手，再预约清理（06-03 原话）。另外**非 2xx 也要关 body**，上面的写法 defer 在检查状态码之前就注册了，正好覆盖。
