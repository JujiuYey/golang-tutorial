...
想让 defer 看到变量的**最终值**？别把变量当参数传，包一层匿名函数（第 03 章学过的闭包），让它执行时再去读：
...
第 03 章学过：命名返回值是函数一进门就存在的变量。`defer` 在 `return` 之后、函数真正退出之前还能运行，所以它可以**改写即将交出去的返回值**：
...
`Close` 的返回值被丢掉了（相当于第 01 篇警告过的 `_` 吞错误）。什么时候能接受？
...
提示：注意 defer 的位置（第 1 节的坑）；带路径的错误用第 02 篇的 `fmt.Errorf("... %s: %w", path, err)`。
