...
把需要的值作为参数传进去,比闭包直接捕获外部变量更清晰——参数在 `go` 那一刻就复制定格了(和 defer 参数的"当场求值"是同一个规则,03-07 篇学过)。
...
想挡住 panic,必须**在那个 goroutine 内部** defer recover(03 模块的知识,跨 goroutine 接不住):
...
老规矩(03 模块讲过):recover 只用来兜底意外崩溃,普通失败该返回 error 或通过 channel 传出去,别用 recover 掩盖。
