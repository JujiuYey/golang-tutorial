...
查一行但一行都没有时,`Scan` 返回专门的哨兵错误 `sql.ErrNoRows`(06 模块学过的 sentinel error,用 `errors.Is` 认):
...
06 模块的 `%w` 包装术直接沿用:报错时能看出"哪个操作、哪个 id"。但注意分寸——**不要把密码、token 这类敏感参数写进错误信息**,错误最终可能进日志、进监控、进告警群。
