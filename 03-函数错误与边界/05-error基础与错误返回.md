...
先扔掉"异常"的概念。Go 的错误就是一个**普通的值**——能赋给变量、能打印、能比较，和一个字符串、一个数字没有本质区别。
...
翻译成大白话：**任何"能把自己说成一句话"的东西，都有资格当错误**（只要它有一个返回 `string` 的 `Error()` 方法）。接口（interface）是后面章节的重头戏，这里先记住这个感性认识就够用了。
